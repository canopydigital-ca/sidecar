<script lang="ts">
  ...
  function reposition() {
      ...
      const desiredW =
        typeof meta.userW === 'number'
          ...
          : typeof meta.defaultW === 'number'
            ? meta.defaultW
            : 380;
      const w = Math.max(minW, Math.min(desiredW, allowedMaxW));
      pop.style.width = `${w}px`;
      pop.style.maxWidth = `${allowedMaxW}px`;
    } else {
      pop.style.width = '';
    ...
    if (placeAbove) {
      pop.style.bottom = `${viewportH - rect.top + 8}px`;
      pop.style.top = 'auto';
...
{#if popoverState.activeKind}
  ...
  <div
    bind:this={popoverEl}
    use:animateIn
    class="fixed z-[9999] bg-[#121212] border border-white/12 rounded-2xl shadow-2xl backdrop-blur-xl outline-none opacity-0 scale-95 transition-all duration-200 ease-[cubic-bezier(0.16,1,0.3,1)] origin-top data-[state=open]:opacity-100 data-[state=open]:scale-100 pointer-events-auto flex flex-col"
    role="dialog"
    aria-modal="false"
  ...
  >
    {#if popoverState.metadata?.resizable}
      ...
      ></button>
    {/if}
    <div class={popoverState.activeKind === 'progressquest' ? 'p-3 overflow-hidden text-white flex-1 min-h-0' : 'p-3 overflow-y-auto overflow-x-hidden text-white flex-1 min-h-0'}>
