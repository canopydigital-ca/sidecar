...
export function isProjectConversationView(): boolean {
  ...
  // DOM heuristic
  if (document.querySelector('[data-testid="project-modal-trigger"]')) return true;
...
function applyProjectWideFix(isWide: boolean) {
  ...
  const composer = document.querySelector('#prompt-textarea') || document.querySelector('textarea[name="prompt-textarea"]');
  if (composer) {
