...
export class PetsLayer {
  ...
  private positionInComposer(container: HTMLElement, settings: PetSettings) {
    const composerEl = (document.querySelector('#prompt-textarea') || document.querySelector('textarea')) as HTMLElement | null;
  ...
  private ensureResizeTargetsObserved(settings: PetSettings) {
    ...
    if (settings.placementMode === 'composer') {
      const composer = document.querySelector('#prompt-textarea') || document.querySelector('textarea');
