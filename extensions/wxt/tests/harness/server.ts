...
// Serve `extensions/wxt/*` at `/...` (this is where the harness fixture lives).
const HARNESS_ROOT = path.resolve(__dirname, '../..');

// Serve `extensions/wxt/.output/chrome/*` at `/wxt/...` (used by wxt-parity.spec).
const WXT_OUT_ROOT = path.resolve(__dirname, '../../.output/chrome');
...
const server = http.createServer((req, res) => {
  try {
    ...
    const abs = resolveSafe(HARNESS_ROOT, pathname);
    if (!abs) return send(res, 403, 'Forbidden');
    return serveFile(res, abs);
  } catch (err) {
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`[harness] serving ${HARNESS_ROOT} at http://127.0.0.1:${PORT}/`);
