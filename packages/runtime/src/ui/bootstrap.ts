...
import { injectConfiguredDocumentCssOnce } from "./styles";
...
function installWindowResizeHook() {
  const w = window as any;
  if (w.__cgpt_window_resize_hook_installed) return;
  w.__cgpt_window_resize_hook_installed = true;

  let scheduled = false;

  const tick = () => {
    scheduled = false;
    void ensureAll("resize");
    const ctx = bootCtx;
    if (ctx) {
      ctx.flags.needDock = true;
      ctx.flags.needInputHandle = true;
      ensureDockInstalled(ctx);
      scheduleWork(ctx, "resize-immediate");
      ctx.scheduleWork("resize");
    }
  };
...
export async function bootFullRuntime() {
  ...
  await safeStep("uiMount", async () => {
    injectConfiguredDocumentCssOnce();
...
async function setupCoreContext() {
  ...
  const ctx: DockContext = {
    ...
    openPopover: (k, b) => popoverManager.toggle(k, b, ctx),
