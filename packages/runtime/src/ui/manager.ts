...
const overlayLocalCss = `
  ...
  /*
   * Per-utility .cgpt-popover descendant remaps (flex/gap/text-size/color/
   * border/rounded/etc.) and the custom animate-spin keyframes were removed
   * here: real Tailwind utilities now reach the shadow root via the @source
   * directive in app.css, making these hand-written duplicates redundant. The
   * element-based popover chrome above (buttons, inputs, headings, scrollbars,
   * resize handle) is intentionally kept -- it is base styling, not a utility
   * remap, and is not fully expressible by the scanned utility classes alone.
   */
`;
...
export class PopoverManager {
  ...
  open(kind: PopoverKind, triggerEl: HTMLElement, ctx: DockContext) {
    // Perf instrumentation is a no-op unless the sidecar debug flag is set
    // (see dev/instrumentation.ts).
    measureInteraction("popover:open", () => this.openInternal(kind, triggerEl, ctx));
  }

  private openInternal(kind: PopoverKind, triggerEl: HTMLElement, ctx: DockContext) {
    try {
