<script lang="ts">
  ...
  onMount(async () => {
    ...
    demoEnabled = (storageService.settings.value as any)?.demoModeEnabled ?? true;
  });
  ...
  async function handleDemoToggle(_event: Event) {
    demoEnabled = !demoEnabled;

    // Persist the setting using storage service
    storageService.settings.update((current: any) => ({
      ...current,
...
{#if isOpen}
  ...
  <!-- Demo Panel -->
  <!-- svelte-ignore a11y_click_events_have_key_events (modal chrome uses click to stop propagation) -->
  <div
