<script lang="ts">
  ...
  type PqIconMap = { nodes: IconNode[] };

  const FALLBACK_ICON_MAP: PqIconMap = {
    nodes: [
      {
        id: 'heavy-helm',
        tags: ['helm', 'armor'],
        visualGroup: 'armor_head',
        path: 'lorc/heavy-helm.svg',
        attributes: { tier: 1 },
        levelRange: { min: 1, max: 10 },
        itemTypes: ['Head Armor']
      },
      {
        id: 'gem-necklace',
        tags: ['amulet', 'jewelry'],
        visualGroup: 'jewelry',
        path: 'lorc/gem-necklace.svg',
        attributes: { tier: 1 },
        levelRange: { min: 1, max: 10 },
        itemTypes: ['Amulet']
      },
      {
        id: 'broadsword',
        tags: ['weapon', 'sword'],
        visualGroup: 'weapons_swords',
        path: 'lorc/broadsword.svg',
        attributes: { tier: 2 },
      ...
      },
      {
        id: 'chest-armor',
        tags: ['armor', 'chest'],
        visualGroup: 'armor_chest',
        path: 'delapouite/chest-armor.svg',
        attributes: { tier: 3 },
        levelRange: { min: 10, max: 20 },
        itemTypes: ['Chest Armor']
      },
      {
        id: 'bordered-shield',
        tags: ['shield', 'armor'],
        visualGroup: 'armor_shields',
        path: 'lorc/bordered-shield.svg',
        attributes: { tier: 1 },
        levelRange: { min: 1, max: 10 },
        itemTypes: ['Shield']
      },
      {
        id: 'boxing-glove',
        tags: ['glove', 'hands'],
        visualGroup: 'armor_hands',
        path: 'lorc/boxing-glove.svg',
        attributes: { tier: 1 },
        levelRange: { min: 1, max: 10 },
        itemTypes: ['Gloves']
      },
  ...
  let pqIconMap = $state<PqIconMap>(FALLBACK_ICON_MAP);

  const PQ_ICON_MAP_CANDIDATES = [
    'vendor/game-icons.net/ffffff/pq-icon-map-v2.json',
    'dist/vendor/game-icons.net/ffffff/pq-icon-map-v2.json',
  ] as const;

  const svgCache = new Map<string, string>();
  const inFlight = new Set<string>();

  async function loadPqIconMapFromAssets(): Promise<void> {
    if (typeof chrome === 'undefined' || !chrome.runtime?.getURL) return;

    for (const relPath of PQ_ICON_MAP_CANDIDATES) {
      try {
        ...
        if (Array.isArray(nodes) && nodes.length > 0) {
          pqIconMap = { nodes: nodes as IconNode[] };
          return;
        }
      } catch {
  ...
  function loadEquipment() {
    const newSlots = slots.map((slot) => {
      ...
      let candidate: IconNode | undefined;
  ...
  onMount(() => {
    void loadPqIconMapFromAssets();
  });

  $effect(() => {
    // Re-run when randomSeed changes
    void randomSeed;
    void pqIconMap.nodes;
    untrack(() => {
      loadEquipment();
    });
  });
