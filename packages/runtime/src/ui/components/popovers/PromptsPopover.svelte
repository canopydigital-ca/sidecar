<script lang="ts">
  ...
  function getInsertTarget(): HTMLElement | null {
    ...
    cachedInsertTarget = getHostAdapter().findComposerEditor() as HTMLElement | null;
  ...
  function getComposerText(): string {
    const editor = getInsertTarget();
    if (!editor) return '';

    if ('value' in editor && typeof (editor as HTMLTextAreaElement).value === 'string') {
