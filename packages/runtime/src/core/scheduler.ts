...
function updateComposerWidth() {
  const anchor = findComposerAnchor(undefined)?.anchor as HTMLElement | null;
  const resizeTarget = findComposerResizeTarget(undefined) as HTMLElement | null; // Pass undefined tracker if lazy
  const widthEl = anchor || resizeTarget || queryFirst("composer.editor") || queryFirst("composer.form") || queryFirst("thread.bottomContainer");
