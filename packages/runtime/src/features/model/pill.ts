...
export async function injectPillModelTrigger(_ctx: DockContext) {
  ...
  btn.addEventListener("click", async (e) => {
    ...
    // Click real selector
    const realSelector = host.findModelPicker() as HTMLElement | null;
    if (!realSelector) {
    ...
    try {
      const menu = await retry(() => {
        ...
        if (!surface) throw new Error("Model menu not visible");
        return surface;
      }, { timeoutMs: 2000, intervalMs: 50, name: "waitForModelMenu" });
