import { DOCK_ID, MODEL_SLOT_ID } from "../../core/constants";
...
export function tagDockMenusAfterClick() {
  window.setTimeout(() => {
    const menus = getHostAdapter().listFloatingMenuSurfaces();
...
export function moveModelPickerIntoDock(tracker?: PerfTracker) {
  ...
  try {
    ...
    if (!slot.dataset.cgptMenuTagBound) {
      slot.dataset.cgptMenuTagBound = "true";
...
function findModelSlot(): HTMLElement | null {
  const lightDomSlot = document.getElementById(MODEL_SLOT_ID);
  if (lightDomSlot instanceof HTMLElement) return lightDomSlot;

  const dock = document.getElementById(DOCK_ID);
  const shadowSlot = dock?.shadowRoot?.getElementById(MODEL_SLOT_ID);
  return shadowSlot instanceof HTMLElement ? shadowSlot : null;
}
